...
func (r *repository) FindByChatIdAndLabel(ctx context.Context, chatId int64, label string) ([]*model.Label, error) {
	q := db.Query{
		Name: "repository.labels.find_by_label",
		Sqlizer: r.sq.Select(labelsTableColumns...).
			From(labelsTable).
			Where(sq.And{
				sq.Eq{labelsTableColumnChatId: chatId},
				sq.Expr("? = ANY("+labelsTableColumnLabels+")", label),
			}),
